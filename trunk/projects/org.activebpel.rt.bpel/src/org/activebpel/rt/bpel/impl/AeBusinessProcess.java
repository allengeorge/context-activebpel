...
import org.activebpel.rt.bpel.impl.queue.*;
 ...
 */
public class AeBusinessProcess extends AeActivityScopeImpl implements IAeBusinessProcessInternal, IAagStatefulProcess, IAeVariableLockCallback, IAeVisitable
{
   // XXX CONDIITON PREDICATE: no-consumer-epr (getConsumerEPR() == null)
   // XXX CONDITION PREDICATE: consumer-epr (getConsumerEPR() != null)
   ...
   /** Reference of the BPELConsumer associated with this process */
   private EndpointReference consumerReference = null;
   /** Map linking each subscription reference to a stateful variable */
   private Map<EndpointReference, IAagStatefulVariable> subscribedVariableMap;
   ...
   protected AeBusinessProcess(long aPid, IAeBusinessProcessEngineInternal aEngine, IAeProcessPlan aPlan)
   {
      ...
      subscribedVariableMap = new HashMap<EndpointReference, IAagStatefulVariable>();
   }
   ...
   /**
    * @see IAagStatefulProcess#queueSubscriptionInvoke(org.activebpel.rt.bpel.IAagSubscriptionInvokeActivity , List<IAagIncomingContextData>)
    */
   ...
   {
      IAagSubscribeInvokeInternal messageQueueObject = createSubscribeQueueObject(aSubsInvoke, aContextParameters);
      getEngine().addSubscribe(getProcessPlan(), messageQueueObject);
   }

   public void queueCreateConsumer() throws AeBusinessProcessException
   {
      IAagConsumerFactoryInvokeInternal messageQueueObject = createConsumerFactoryQueueObject();
      getEngine().addCreateConsumer(getProcessPlan(), messageQueueObject);
   }
   ...
   // XXX BLOCKS UNTIL: consumer-epr
   protected synchronized IAagSubscribeInvokeInternal createSubscribeQueueObject(IAagSubscriptionInvokeActivity aSubsInvoke,
      List<IAagIncomingContextData> contextParams)
      throws AeBusinessProcessException
   {
      if (getConsumerEPR() == null) {
         queueCreateConsumer();
      }

      while (getConsumerEPR() == null) {
         try {
            wait();
      ...
      IAagSubscribeInvokeInternal messageQueueObject;

      messageQueueObject = new AagSubscribe(getProcessId(),
                                            getName(),
                                            aSubsInvoke,
                                            contextParams,
                                            consumerReference);
      messageQueueObject.setSubscriptionInvokeActivity(aSubsInvoke);

      return messageQueueObject;
   }

   public void addSubscription(EndpointReference subscriptionEPR, IAagStatefulVariable statefulOutputVariable) {
      System.out.println("Adding subscription reference:\n" + subscriptionEPR.toString() + "to variable "
            + ((IAeVariable) statefulOutputVariable).getName() + " to map");
      subscribedVariableMap.put(subscriptionEPR, statefulOutputVariable);
   }

   protected IAagConsumerFactoryInvokeInternal createConsumerFactoryQueueObject() {
      String engineURL = getEngine().getBaseInvocationURL();

      IAagConsumerFactoryInvokeInternal messageQueueObject;
      messageQueueObject = new AagCreateConsumer(getProcessId(), getName(), engineURL, this);
   ...
   public synchronized void dispatchSubscribeData(String aLocationPath, IAagSubscribeResponse subsResponse)
   {
      try
      {
         // find the receiver and if the receiver is not found then throw an
         // exception
         IAagSubscriptionReceiver subsReceiver = (IAagSubscriptionReceiver) findBpelObjectOrThrow(aLocationPath);
         // dispatch the invoke return if throws an exception while processing then fault the bpel object
         try
         {
            getExecutionQueue().dispatchSubscribeData(subsReceiver, subsResponse);
         }
         catch (AeBusinessProcessException e)
         {
            objectCompletedWithFault((IAeBpelObject) subsReceiver, AeFaultFactory.getSystemErrorFault(e));
         }
      }
      catch (AeBusinessProcessException e)
      {
   ...
   public synchronized void setConsumerEPR(EndpointReference consumerEPR) {
      this.consumerReference = consumerEPR;
      notifyAll();
   }

   // Not strictly necessary since I'm not calling any condition queue methods. That said, this is probably better
   // synchronized because an EPR cannot be replaced in an atomic operation
   public synchronized EndpointReference getConsumerEPR() {
      return consumerReference;
   }

   public void updateStatefulVariable (EndpointReference subscriptionReference, Element newValueElement) {
      IAagStatefulVariable var = subscribedVariableMap.get(subscriptionReference);
      if (var != null) {
         try {
            System.out.println("passing variable value to stateful variable");
            var.updateValue(subscriptionReference, newValueElement);
         } catch (AeUninitializedVariableException e) {
            System.err.println("error: can't propagate notifications of changed values to an unitialized variable");
         }
      } else {
         System.err.println("can't find variable in subscribed variable map");
      }
   }
   ...
   /**
    * @see org.activebpel.rt.bpel.impl.IAagStatefulProcess#isStateful()
    */
   public boolean isStatefulProcess()
   {
      AeProcessDef processDef = getProcessDefinition();
      return processDef.isStatefulProcess();
   }
