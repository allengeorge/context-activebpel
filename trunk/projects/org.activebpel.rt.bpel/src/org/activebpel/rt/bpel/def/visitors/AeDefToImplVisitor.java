// $Header: /Development/AEDevelopment/projects/org.activebpel.rt.bpel/src/org/activebpel/rt/bpel/def/visitors/AeDefToImplVisitor.java,v 1.42 2007/09/12 02:48:1
/*
...
/**
 * Visitor responsible for creating implementation objects from their definition
 * objects.<p>
 *
 * A static convenience method called createProcess has been setup to easily
...
public abstract class AeDefToImplVisitor implements IAeDefToImplVisitor
{
   ...
   public void visit(AeActivityReceiveDef aDef)
   {
      ...
      assignMessageDataConsumer(impl, new AeActivityReceiveConsumerContext(impl), aDef);
   ...
   public void visit(AeActivityReplyDef aDef)
   {
      ...
      assignMessageDataProducer(impl, new AeActivityReplyProducerContext(impl), aDef);
   ...
   public void visit(AeOnMessageDef aDef)
   {
      ...
      assignMessageDataConsumer(msg, new AeOnMessageConsumerContext(msg), aDef);
   ...
   {
   // fixme (MF) seems like this should throw
      traverse(aDef, null);
   }
    ...
    * @param aContext - context for the message producer
    * @param aDef - def provides the strategy
    */
   protected void assignMessageDataProducer(IAeWSIOActivity aImpl, IAeMessageDataProducerContext aContext, IAeMessageDataProducerDef aDef)
   {
      ...
      {
         messageDataProducer = new AeVariableMessageDataProducer(aContext);
      }
      else if (IAeMessageDataStrategyNames.TO_PARTS.equals(producerStrategy))
      {
         messageDataProducer = new AeToPartsMessageDataProducer(aContext);
      }
      else
      {
         ...
         messageDataProducer = new AeEmptyMessageDataProducer(aContext);
      }
    ...
    * @param aDef
    */
   protected void assignMessageDataConsumer(IAeWSIOActivity aImpl, IAeMessageDataConsumerContext aContext, IAeMessageDataConsumerDef aDef)
   {
      ...
      {
         messageDataConsumer = new AeVariableMessageDataConsumer(aContext);
      }
      else if (IAeMessageDataStrategyNames.FROM_PARTS.equals(aDef.getMessageDataConsumerStrategy()))
      {
         messageDataConsumer = new AeFromPartsMessageDataConsumer(aContext);
      }
      else
      {
         messageDataConsumer = new AeNoopMessageDataConsumer(aImpl);
      }
