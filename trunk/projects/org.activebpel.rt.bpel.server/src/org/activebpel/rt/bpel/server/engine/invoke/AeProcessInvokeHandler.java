// $Header: /Development/AEDevelopment/projects/org.activebpel.rt.bpel.server/src/org/activebpel/rt/bpel/server/engine/invoke/AeProcessInvokeHandler.java,v 1.18 2007/06/18 13:25:4
/*
...
public class AeProcessInvokeHandler implements IAeTwoPhaseInvokeHandler, IAeMessageAcknowledgeCallback
{
   ...
   protected void initialize(IAeInvoke aInvoke) throws AeException
   {
      ...
      if ( isPersistent() )
      {
         getTransmissionTracker().assignTransmissionId(aInvoke);
      }
   }
   ...
   public IAeWebServiceResponse handleInvoke(IAeInvoke aInvoke, String aQueryData ) 
   {
      ...
      // wait until delivered (engine will call back once the message has been delivered).
      if ( !isDelivered() )
      {
         synchronized (this)
         {
            if ( !isDelivered() )
            {
               ...
               long wsTimeoutMs = config.getWebServiceTimeout() * 1000L;
               try
               {
                  wait(wsTimeoutMs);
               }
   ...
   {
      // journal the acknowlegement. The aMessageId is normally null for subprocess invoke, but
      // not neccesarilly null for other durable invokes such as WS-RM.
      try
      {
    ...
    * @param aResponse
    * @param aServiceName
    * @param aQueueForExecution if a process was created, then queue it for execution.
         ...
         throws AeException
   {
      ...
      try
      {
         ...
         partnerEndpointRef.getPolicies().clear();
      }
