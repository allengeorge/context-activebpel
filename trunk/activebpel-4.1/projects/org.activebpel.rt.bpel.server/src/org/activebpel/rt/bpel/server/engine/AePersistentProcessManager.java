// $Header: /Development/AEDevelopment/projects/org.activebpel.rt.bpel.server/src/org/activebpel/rt/bpel/server/engine/AePersistentProcessManager.java,v 1.79 2007/07/31 00:51:2
/*
...
public class AePersistentProcessManager extends AeAbstractProcessManager implements IAePersistentProcessManager, IAeProcessWrapperMapCallback, IAeRecoverableProcessManager
{
    ...
    * <em>Each call to {@link #getProcessWrapperWithMutex(long)} or {@link #getProcessWrapperWithMutex(long)}
    * must be followed eventually by a matching call to {@link releaseProcessWrapperWithMutex(AeProcessWrapper)}</em>.
    *
    * @param aProcessId
    */
   ...
   /**
    * @see org.activebpel.rt.bpel.impl.IAeProcessManager#journalInboundReceive(long, int, org.activebpel.rt.bpel.impl.queue.AeInboundReceive, boolean)
    */
   ...
   protected void saveProcess(AeProcessWrapper aWrapper) throws AeBusinessProcessException
   {
      ...
      // List of transmissionds Ids that needs to be deleted.
      ...
      // completed journal entry or transmission id, then save the process.
      if (aWrapper.isModified() || !completedJournalIds.isEmpty() || !completedTransmissionIds.isEmpty())
      {
         // Save the process and delete the completed journal entries.
         int n = getProcessStateWriter().writeProcess(aWrapper.getProcess(), completedJournalIds, completedTransmissionIds);
