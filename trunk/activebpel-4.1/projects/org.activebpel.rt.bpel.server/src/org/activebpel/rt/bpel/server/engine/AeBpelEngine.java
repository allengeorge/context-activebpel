// $Header: /Development/AEDevelopment/projects/org.activebpel.rt.bpel.server/src/org/activebpel/rt/bpel/server/engine/AeBpelEngine.java,v 1.101 2007/08/15 21:02:0
/*
...
public class AeBpelEngine extends AeAbstractServerEngine
{
   ...
   public boolean registerForCoordination(IAeMessageContext aContext, IAeBusinessProcess aProcess) throws AeBusinessProcessException
   {
      ...
      // create a coordination registation request.
   ...
   private static class AeDispatchReceiveRunnableHelper extends AeAbstractWork implements IAeInputMessageWork
   {
      ...
      {
         // We can restore the inbound receive only from a persistent process
         // manager.
         IAeProcessManager pm = getBpelEngine().getProcessManager();
         ...
         // Get the process to deserialize the inbound receive from its journal
         // entry.
         IAeBusinessProcess process = getBpelEngine().getProcessById(mProcessId);

         try
         {
            IAePersistentProcessManager ppm = (IAePersistentProcessManager) pm;

            // Read the journal entry for the inbound receive. If the journal
            // entry has already been consumed, then this will return null.
            AeInboundReceiveJournalEntry entry = (AeInboundReceiveJournalEntry) ppm.getJournalEntry(mJournalId);
            if (entry == null)
            {
               throw new AeBusinessProcessException(AeMessages.getString("AeBpelEngine.ERROR_MissingInboundReceiveJournalEntry")); //$NON-NLS-1$
            }
         ...
         finally
         {
            ...
            getBpelEngine().releaseProcess(process);
         }
      }
   }
   ...
   /**
    * Creates the process and sets the starting message on it. There's special
    * handling for multi-start processes due to the possibility of simultaneous
    * arrival of start messsages.
    */
    ...
    * @see org.activebpel.rt.bpel.impl.AeBusinessProcessEngine#internalCreateProcessWithMessage(org.activebpel.rt.bpel.impl.queue.AeInboundReceive, org.activebpel.wsio.IAeMessageAcknowledgeCallback)
    */
   protected IAeBusinessProcess internalCreateProcessWithMessage(AeInboundReceive aInboundReceive, IAeMessageAcknowledgeCallback aAckCallback) throws AeBusinessProcessException
   {
      ...
      try
      {
         process = super.internalCreateProcessWithMessage(aInboundReceive, aAckCallback);
