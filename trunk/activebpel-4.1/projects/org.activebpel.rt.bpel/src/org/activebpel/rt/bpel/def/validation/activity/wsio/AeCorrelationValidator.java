//$Header: /Development/AEDevelopment/projects/org.activebpel.rt.bpel/src/org/activebpel/rt/bpel/def/validation/activity/wsio/AeCorrelationValidator.java,v 1.8 2007/01/27 14:39:3
/*
...
public class AeCorrelationValidator extends AeBaseValidator
{
   ...
   public void validate()
   {
      ...
      if (getSetModel() != null)
      {
         ...
         {
            getReporter().addError( ERROR_CORRELATION_PATTERN_NOT_ALLOWED,
                  null, getDefinition() );
         }
         else if (correlationUser.isPatternRequired() && !hasPattern)
         {
            getReporter().addError( ERROR_CORRELATION_PATTERN_REQUIRED,
                  null, getDefinition() );
         }
         else if (hasPattern)
         {
   ...
   protected void recordPropertyUsage(AeMessagePartsMap aMap)
   {
      ...
      for(Iterator it=getSetModel().getDef().getPropertiesList(); it.hasNext();)
      {
         ...
         if (propAlias == null)
         {
            getReporter().addError( AeMessages.getString("AeProcessDef.MissingPropertyAlias"), //$NON-NLS-1$
                  new Object[] {
         ...
         else 
         {
            ...
            try
            {
               if (propAlias.getType() == IAePropertyAlias.MESSAGE_TYPE)
               {
                  ...
                  if (part == null)
                  {
                     getReporter().addError( ERROR_PROPERTY_ALIAS_BAD_PART,
                  ...
                  else
                  {
                     ...
                     {
                        getReporter().addError( ERROR_NO_QUERY_FOR_PROP_ALIAS,
                              new String[] { getNSPrefix(aMap.getMessageType().getNamespaceURI()),
                                             aMap.getMessageType().getLocalPart(),
                                             getNSPrefix(propName.getNamespaceURI()),
                                             propName.getLocalPart(),
            ...
            {
               getReporter().addWarning(AeVariableValidator.ERROR_INVALID_XPATH, new String[] { propAlias.getQuery(),
