...
public class AeBusinessProcessEngine implements IAeBusinessProcessEngineInternal, IAeBusinessProcessEngineCallback
{
   ...
   private String mBaseInvocationURL = null;
         ...
         IAeMessageContext aContext) throws AeBusinessProcessException
   {
      ...
      // XXX I think this is the last place at which I have the wsa headers!!!
      System.out.println(aContext.getWsAddressingHeaders().getRecipient().getAddress());
      ...
      // XXX At this point all I have is a pID
   ...
   protected long queueReceiveDataInternal(AeInboundReceive aInboundReceive, IAeMessageAcknowledgeCallback aAckCallback, boolean aQueueForExecution)
         throws AeCorrelationViolationException, AeConflictingRequestException, AeBusinessProcessException
   {
      // XXX AeInboundReceive has the message context of the receive stored in it, so you can use:
      // XXX AeInboundReceive.getContext().getWsAddressingHeaders().getRecipient().getAddress() to get the URI
   ...
   protected long createProcessWithMessage(AeInboundReceive aInboundReceive, IAeMessageAcknowledgeCallback aAckCallback, boolean aQueueForExecution)
      throws AeBusinessProcessException
   {
      // XXX I still have the message headers
   ...
   public void queueCreateConsumerData(long processID, QName processQName, EndpointReference processWSNConsumerEPR) {
      ...
      // FIXME Not doing any journaling
      try
      {
         process = getProcessById(processID);
         process.setConsumerEPR(processWSNConsumerEPR);
   ...
   public void queueCreateConsumerFault() {
      // FIXME Unimplemented queueCreateConsumerFault
   }
   ...
   public void queueSubscribeData(long processId, QName processName, String locationPath, IAagSubscribeResponse subsResponse) {
      ...
      try
      {
         ...
         // invoke for the given path must exist and must be in an executing state and the tranmission ids must match.
         // if the invoke cannot be matched, then matchedInvoke method will throw a bus-process-exception.
         if ( matchesSubscribeInvoke(processId, locationPath, invoke) )
         {
            process.dispatchSubscribeData(locationPath, subsResponse);
         }
         else
         {
            Object[] args = {String.valueOf(processId), locationPath};
            throw  new AeBusinessProcessException(AeMessages.format("AeBusinessProcessEngine.UNMATCHED_INVOKE", args));
         }
      } catch (AeBusinessProcessException e) {
         e.printStackTrace(System.err);
      } finally {
   ...
   public void queueSubscribeFault() {
      // FIXME Unimplemented queueSubscribeFault
   }
   ...
   public void updateStatefulVariable(long processID, EndpointReference subscriptionReference, Element newValueElement) {
      ...
      try {
         process = getProcessById(processID);
         process.updateStatefulVariable(subscriptionReference, newValueElement);
      } catch (AeBusinessProcessException e) {
         e.printStackTrace(System.err);
      }  finally {
         releaseProcess(process);
   ...
   /**
    * Does not check for transaction ID, unlike matchesInvoke
    */
   protected boolean matchesSubscribeInvoke(long aProcessId, String aLocationPath, IAeInvokeActivity aInvoke) throws AeBusinessProcessException
   {

      // check if the invoke is currently executing
      if (aInvoke != null && aInvoke.isExecuting())
      {
         return true;
      }
      else
   ...
   /**
    * @see org.activebpel.rt.bpel.impl.IAeBusinessProcessEngineInternal#addSubscribe(org.activebpel.rt.bpel.impl.IAeProcessPlan, org.activebpel.rt.bpel.impl.IAeInvokeInternal)
    */
   ...
   {
      getQueueManager().addSubscribe(aPlan, aSubsInvokeQueueObject);
   }
   ...
   {
      //FIXME Put get process by id here
      getQueueManager().addConsumerFactoryInvoke(aPlan, aConsumerFactoryInvokeQueueObject);
   }
   ...
   /**
    * @see org.activebpel.rt.bpel.impl.IAeBusinessProcessEngineInternal#getBaseInvocationURL() 
    */
   public String getBaseInvocationURL() {
      return mBaseInvocationURL;
   }

   /**
    * @see org.activebpel.rt.bpel.impl.IAeBusinessProcessEngineInternal#setBaseInvocationURL(String)
    */
   public void setBaseInvocationURL(String baseURL) {
      mBaseInvocationURL = baseURL;
   }
}
