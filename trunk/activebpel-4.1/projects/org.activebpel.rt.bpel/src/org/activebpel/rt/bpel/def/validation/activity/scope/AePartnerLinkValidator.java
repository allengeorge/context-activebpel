//$Header: /Development/AEDevelopment/projects/org.activebpel.rt.bpel/src/org/activebpel/rt/bpel/def/validation/activity/scope/AePartnerLinkValidator.java,v 1.7 2007/06/08 20:17:5
/*
...
public class AePartnerLinkValidator extends AeBaseValidator
{
   ...
   public void validate()
   {
      ...
      if (!isReferenced())
      {
         getReporter().addWarning( WARNING_PARTNER_LINK_NOT_USED,  new String[] { getDef().getName() }, getDef() );
      }
      ...
      if (hasMyRole())
      {
         ...
         {
            // role not found
            getReporter().addError( ERROR_ROLE_NOT_FOUND,
      ...
      else
      {
         ...
         {
            // role not found
            getReporter().addError( ERROR_INIT_PARTNER_ROLE_NOT_ALLOWED,
   ...
   protected void validatePortType(IAePartnerLinkOperationUser aUser, IAeRole aRole)
   {
      ...
      {
         getReporter().addError( ERROR_ROLE_HAS_NO_PORTTYPE,
               new String[] { getDef().getMyRole(), getPartnerLinkType().getName() },
               getDefinition() );
      }
      else if (!portTypesMatch(aUser.getPortType(), aRole.getPortType().getQName()))
      {
         ...
         getReporter().addError( ERROR_PORTTYPE_MISMATCH,
