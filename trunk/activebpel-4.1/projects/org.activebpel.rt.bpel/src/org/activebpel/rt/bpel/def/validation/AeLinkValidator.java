// $Header: /Development/AEDevelopment/projects/org.activebpel.rt.bpel/src/org/activebpel/rt/bpel/def/validation/AeLinkValidator.java,v 1.17 2007/03/03 02:51:1
/*
...
public class AeLinkValidator implements IAeValidationDefs
{
    ...
    */
   public AeLinkValidator( AeProcessDef aProcessDef, IAeBaseErrorReporter aReporter )
   {
    ...
    * @param aReporter The reporter to set.
    */
   public void setReporter(IAeBaseErrorReporter aReporter)
   {
      mReporter = aReporter;
   }
   ...
   public void checkLinks()
   {
      for ( Iterator iter = getLinks().iterator() ; iter.hasNext() ; )
      {
         ...
         if (comp.isUsed())
         {
            ...
            {
               getReporter().addError( ERROR_BAD_LINK,
         ...
         {
            getReporter().addWarning( WARNING_LINK_NOT_USED,
   ...
   private void checkLinkCycles()
   {
      ...
      for (Iterator iter = cycleLinks.iterator(); iter.hasNext(); )
      {
         ...
         getReporter().addError( ERROR_LINK_CYCLE, new String[]{link.getName()}, link );
      }
   }
   ...
   private void checkBoundaryCrossings( AeLinkComposite aLink )
   {
      ...
      {
         getReporter().addError( ERROR_LINK_CROSSING,
   ...
   private void checkBadTarget( AeLinkComposite aLink )
   {
      ...
      for(AeBaseDef dstModel = dst; dstModel != null; dstModel = dstModel.getParent() )
      {
         if ( dstModel == src )
         {
            getReporter().addError( ERROR_SCOPE_LINK, new String[] {aLink.getLink().getLocationPath()},
                                    aLink.getLink() );
            break;
         }
      }
   ...
   public void addSource( AeSourceDef aSource, AeActivityDef aDef )
   {
      ...
      if ( link != null )
      {
         ...
         if ( comp.mSource != null)
         {
            ...
            getReporter().addError( ERROR_MULTI_SRC_LINK,
                                    new String[] { comp.getLink().getLocationPath() },
                                    comp.getLink() );
            return;
         }
   ...
   public void addTarget( AeTargetDef aTarget, AeActivityDef aDef )
   {
      ...
      if ( link != null )
      {
         ...
         if ( comp.mTarget != null)
         {
            ...
            getReporter().addError( ERROR_MULTI_TARGET_LINK,
                                    new String[] { comp.getLink().getLocationPath() },
                                    comp.getLink() );
            return;
         }
