// $Header: /Development/AEDevelopment/projects/org.activebpel.rt.bpel/src/org/activebpel/rt/bpel/impl/queue/AeInvoke.java,v 1.22 2007/05/01 17:13:1
/*
...
public class AeInvoke extends AeAbstractQueuedObject implements IAeInvokeInternal
{
   ...
   /**
    * Accessor for the parnter reference endpoint.
    */
   ...
   /**
    * Overrides method to set the transmission
    * @see org.activebpel.rt.bpel.impl.IAeInvokeInternal#setRecoveredTransmissionId(long)
    */
   public void setRecoveredTransmissionId(long aTransmissionId)
   {
      //
      // During recovery, set/force the transmission id recovered from journal entry
      //
      // (Note: calling setTransmissionId(long) will not work during recovery since
      // it checks to see if the tx-id is current. The tx-id is normally not current
      // during recovery. )
      //

      // update the local copy
      mTransmissionId = aTransmissionId;
      // update the value in the AeActivityInvokeImpl.
      ...
      {
         // Log warning -  should not get here!
         AeException.logWarning("Cannot set recovered transmission id - AeActivityInvokeImpl is null"); //$NON-NLS-1$
      }
   }
